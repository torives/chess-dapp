...
# Use docker-compose to start a testnet validator node for the DApp in production mode
...
compose_down () {
  echo "Shutting down validator node..."
  docker compose --env-file ./testnet/env.$network -f ./testnet/validator.yml down -v
}

# set the trap for SIGINT signal (CTRL+C)
trap compose_down INT

# run validator node
docker compose --env-file ./testnet/env.$network -f ./testnet/validator.yml up
